...
function render_pipeline:__init(scene)
    ...
    self.volumetric_clouds_shader_ = FileSystem:search("shaders/volumetric_clouds.glsl", true)
...
function render_pipeline:create_hdr_render(planet_camera, sector_camera, occlusion_tex)
	...
	local generate_volumetric_clouds = function()
        ...
        if self.scene_.planet_.clouds_.is_loaded_ then
        
            self.volumetric_clouds_shader_:bind()	
            root.Shader.get():uniform("u_InvProjMatrix", math.inverse(math.perspective(sector_camera.fov, sector_camera.aspect, 1.0, 100.0)))
            root.Shader.get():uniform("u_InvRotation", mat3(math.inverse(sector_camera.view_matrix)))
            
            local earth_radius = self.scene_.planet_.clouds_:calculate_planet_radius(self.scene_.planet_.clouds_.atmosphere_start_height_, self.scene_.planet_.clouds_.horizon_distance_)
            root.Shader.get():uniform("_CameraPosition", sector_camera.position + vec3(0, earth_radius, 0))
            self.scene_.planet_.clouds_:set_uniforms()
...
function render_pipeline:render(planet_camera, sector_camera)
