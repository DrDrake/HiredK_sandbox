...
function client:__init(w, h, title, style)
    ...
    self.planet_ = planet()
    self.planet_:load(6360000.0, 14, 5123)
    
    self.camera_ = camera_first_person(math.radians(90), w/h, 0.1, 15000000.0)
    ...
    self.player_ = bt.CharacterController(self.dynamic_world_, 1.5, 0.2)
    self.player_:wrap(vec3(156.1798086299, 40, 700.42817127732))
    ...
    for i = 1, table.getn(sector.objects_) do
        sector.objects_[i]:__on_physics_start(self.dynamic_world_)
    end
end
...
function client:screen_to_world_ray(x, y)
    ...
    local inv_proj = math.inverse(self.camera_.proj_matrix)
    local inv_view = math.inverse(self.camera_.view_matrix)
    ...
    ray.direction = ray.direction * self.camera_.clip.y
    return ray
end
...
function client:process_view(dt)
    ...
    if self:has_focus() then
        ...
        local speed = (sf.Keyboard.is_key_pressed(sf.Keyboard.LShift) and 2.5 or 0.15)
        local direction = vec3()
        
        if sf.Keyboard.is_key_pressed(sf.Keyboard.W) then
            direction = direction - vec3(math.column(self.camera_.local_, 2)) * speed
        end		
        if sf.Keyboard.is_key_pressed(sf.Keyboard.S) then
            direction = direction + vec3(math.column(self.camera_.local_, 2)) * speed
        end
        if sf.Keyboard.is_key_pressed(sf.Keyboard.D) then
            direction = direction + vec3(math.column(self.camera_.local_, 0)) * speed
        end
    ...
    self.camera_.position = self.player_.position + vec3(0, 2.0, 0)
...
function client:__update(dt)
    ...
    self.render_pipeline_:render(dt, self.camera_)
	
	--self.dynamic_world_:draw(self.camera_)
    self:display()
end
