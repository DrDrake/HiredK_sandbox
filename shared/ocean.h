...
class 'ocean' (root.ScriptObject)

function ocean:__init()
    root.ScriptObject.__init(self, "ocean")
    ...
    self.shader_ = FileSystem:search("shaders/gbuffer_ocean.glsl", true)
	...
	local slope_variance_tex_settings_ = { target = gl.TEXTURE_3D, iformat = gl.R16F, format = gl.RED, type = gl.FLOAT, filter_mode = gl.LINEAR }
    ...
    self.screen_grid_ = root.Mesh.build_plane(320, 180, true, false)
    self.screen_quad_ = root.Mesh.build_quad()
	self:compute_slope_variance_tex()
end

function ocean:compute_slope_variance_tex()
...
function ocean:draw(camera, radius)
	...
	gl.ActiveTexture(gl.TEXTURE0)
	root.Shader.get():sampler("fftWavesSampler", 0)
	self.ffta_tex_:bind()
	
	gl.ActiveTexture(gl.TEXTURE1)
	root.Shader.get():sampler("slopeVarianceSampler", 1)
